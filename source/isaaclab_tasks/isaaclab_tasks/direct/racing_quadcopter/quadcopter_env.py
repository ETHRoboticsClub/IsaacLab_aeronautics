...
from .curriculums import check_collision_curriculum_enabled
...
class RacingQuadcopterEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Check for crashes (ground collision or upside down) - simple checks for rewards
        ...
        # Track collision count for logging (using termination flag from _get_dones)
        self._episode_collision_count += self.reset_terminated.float()
    ...
    def _check_collision(self) -> torch.Tensor:
        """Check for collisions using contact sensor forces with per-environment curriculum.
        
        Returns:
            torch.Tensor: Boolean tensor [N] indicating which environments have collisions
        """
        
        # Update curriculum state using SAVED metrics from previous episode
        self._collision_detection_enabled = check_collision_curriculum_enabled(
            env=self,
            gates_passed_per_env=self._last_episode_gates_passed,
            passage_velocity_per_env=self._last_episode_mean_velocity,
            passage_centering_per_env=self._last_episode_center_offset,
            passage_count_per_env=torch.ones_like(self._last_episode_gates_passed),  # Already computed as means
            min_gates_passed=self.cfg.curriculum_min_gates_passed,
            min_mean_velocity=self.cfg.curriculum_min_mean_velocity,
            max_center_offset=self.cfg.curriculum_max_center_offset
        )
        ...
        # Compute collision for all environments
        collision_detected = torch.any(
            torch.max(
                torch.norm(net_contact_forces[:, :, self._undesired_contact_body_ids], dim=-1),
                dim=1
            )[0] > self.cfg.collision_force_threshold,
            dim=1
        )
    ...
    def _reset_idx(self, env_ids: torch.Tensor) -> None:
        ...
        if len(env_ids) > 0:
            ...
            # --- Save per-environment metrics for curriculum (BEFORE reset) ---
            self._last_episode_gates_passed[env_ids] = self._gates_passed[env_ids].float()
            
            # Compute per-env mean velocity and center offset from reward manager
            has_passages = self._reward_manager._passage_count[env_ids] > 0
            self._last_episode_mean_velocity[env_ids] = torch.where(
                has_passages,
                self._reward_manager._passage_velocity_sum[env_ids] / self._reward_manager._passage_count[env_ids],
                torch.zeros_like(self._reward_manager._passage_velocity_sum[env_ids])
            )
            self._last_episode_center_offset[env_ids] = torch.where(
                has_passages,
                self._reward_manager._passage_centering_sum[env_ids] / self._reward_manager._passage_count[env_ids],
                torch.zeros_like(self._reward_manager._passage_centering_sum[env_ids])
            )
